  ...
  create  table
    "divvy"."public_gold"."trips_enhanced__dbt_tmp"
...
WITH trip_weather AS (
  SELECT
    ...
    CASE 
      WHEN t.member_casual = 'member' THEN
        ...
        CASE 
          ...
          ELSE CEILING(t.ride_length_minutes - 45) * 0.19
        END
      WHEN t.member_casual = 'casual' THEN
        ...
        CASE 
          ...
          ELSE CEILING(t.ride_length_minutes - 180) * 0.19
        END
      ELSE 0
    END AS overage_fee,
    ...
    CASE 
      WHEN t.ride_length_minutes > 1440 THEN 250.0
      ELSE 0
    END AS lost_stolen_fee,
    
    -- Estimated base revenue (for casual riders only - members pay annually)
    CASE 
      WHEN t.member_casual = 'casual' THEN 18.1
      ELSE 0
    END AS base_revenue

  FROM "divvy"."public_silver"."trips_cleaned" t
  LEFT JOIN "divvy"."public_silver"."weather_cleaned" w ON t.ride_date = w.weather_date
  LEFT JOIN "divvy"."public_silver"."stations_cleaned" ss ON t.start_station_id = ss.station_id
  LEFT JOIN "divvy"."public_silver"."stations_cleaned" es ON t.end_station_id = es.station_id
),
...
final_calculations AS (
  SELECT
    *,
    ...
    (base_revenue + overage_fee + lost_stolen_fee) * 0.1025 AS sales_tax,
    
    -- Final revenue including tax
    (base_revenue + overage_fee + lost_stolen_fee) * (1 + 0.1025) AS total_trip_revenue_with_tax,
